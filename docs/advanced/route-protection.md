...
In order to configure the protection of routes, the [login](#login) decorator of the created object of the app is used. The utility of this decorator is to obtain the values that we have previously registered in the `client_storage` [(more details here)](https://docs.flet.dev/cookbook/client-storage/), then you can perform validations with a database or any data manager used.

Decorator to add a login configuration to the app [`protected_route`](../started/how-to-use.md#decorator-page):

* The decorator function must receive a mandatory parameter, for example: [`data:fs.Datasy`](../guide/core/datasy.md). Which can be used to get information and perform universal settings of the app.
...
**Parameters [`data.login`](../started/how-to-use.md#methods_1):**
...
* `time_expiry` : Time to expire the session, use the `timedelta` class  to configure. (Optional) ([JWT usage required](basic-jwt.md))
* `sleep` : Time to do login checks, default is 1s. (Optional) ([JWT usage required](basic-jwt.md))
