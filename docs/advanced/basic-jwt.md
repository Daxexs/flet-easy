...
In the [`FletEasy`](../guide/core/fletEasy.md) class we must configure the following parameters:
...
After having configured the `secret_key`, we can start configuring the use of JWT. For this we are going to require the use of the [`login`](route-protection.md) method of `Datasy` (data), it will be used as normally we would use it without using JWT, but we will use the `time_expiry` and `value` parameter that will have to be a dictionary obligatorily.

* [More details of the `login` method](route-protection.md#login)
* [More details of the `logout` method](route-protection.md#logout)
...
def dashboard_page(data: fs.Datasy):
    return ft.View(
        controls=[
            ...
            ft.ElevatedButton(
                "Logout",
                on_click=lambda _: data.logout("login")
                ),
            ft.ElevatedButton(
                "Home",
...
**APP**
<video controls>
  <source src="../../assets/advanced/basic-jwt-app.webm" type="video/webm" alt="Flet-Easy - basic-jwt-app">
</video>
